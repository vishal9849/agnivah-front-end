...
<script lang="ts">
  ...
  // import { user } from "../stores/authStore"; // Adjust the path as needed
  ...
  function handleClickOutside(event: MouseEvent) {
    ...
    if (
      dropdownConformationRef &&
      !dropdownConformationRef.contains(event.target as Node)
    ) {
  ...
  export let navLinks = [
    { text: "CV-Analyzer", href: "/cv-analyzer" },
    { text: "About", href: "/about" },
    { text: "Home", href: "/unauthorized" },
    { text: "Services", href: "/unauthorized" },
    { text: "Contact", href: "/unauthorized" },
    { text: "Login", href: "/unauthorized" },
  ];
...
{#if showConfirmation}
  <div
    class="fixed top-0 left-0 w-full h-full bg-opacity-50 z-20 flex items-center justify-center"
  >
    <div
      class="bg-gray-800 border border-gray-900 rounded-lg shadow-xl p-6 transition-all ease-in-out duration-600"
    >
      ...
      <div class="flex justify-center gap-4">
        ...
        text-gray-100 font-bold py-2 px-4 rounded transition-all ease-in-out duration-600"
          onclick={confirmSignOut}>Yes, Sign Out</button
        >
        <button
          class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded tranalletion-all ease-in-out duration-600"
          onclick={() => (showConfirmation = !showConfirmation)}>No</button
        >
      </div>
    </div>
  </div>
          ...
          <div class="relative" bind:this={dropdownUserRef}>
            ...
            {#if showUserMenu}
              ...
              >
                <!-- Email on top -->
                <div
                  class="text-sm text-gray-100 mb-3 font-semibold text-center"
                >
                  <!-- {user?.role} -->
                </div>
