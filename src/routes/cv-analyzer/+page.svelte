<script lang="ts">
  ...
  const ENHANCING_ENDPOINT = import.meta.env.VITE_API_ENHACING_ENDPOINT;
  const DOWNLOADING_ENDPOINT = import.meta.env.VITE_API_DOWNLOADING_ENDPOINT;
  // console.log(
  //   "ENHACING_ENDPOINT",
  //   ENHANCING_ENDPOINT,
  //   "DOWNLOADING_ENDPOINT",
  //   DOWNLOADING_ENDPOINT
  // );
  ...
  async function analyzeCV() {
    ...
    try {
      const response = await fetch(`${ENHANCING_ENDPOINT}`, {
        method: "POST",
        body: formData,
      });
    ...
    return analysisResult
      ...
      : {
          ...
          enhanced_cv: "No enhanced CV available",
        };
  }
    ...
    >
      ...
      <div class="mb-4">
        ...
        {#if isAnalyzeButtonDisabled && jobDescription === ""}
          <p class="text-red-400 text-sm mt-1">Job description is required.</p>
        {/if}
      </div>
